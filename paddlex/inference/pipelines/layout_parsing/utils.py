...
def get_overlap_boxes_idx(src_boxes, box):
    """get overlap boxes idx"""

    match_idx_list = []
    use_crop_ocr = False
    src_boxes_num = len(src_boxes)
    if src_boxes_num > 0 and len(box) > 0:
        box_w = box[2] - box[0]
        boxes_w = src_boxes[:, 2] - src_boxes[:, 0]
        x1 = np.maximum(box[0], src_boxes[:, 0])
        y1 = np.maximum(box[1], src_boxes[:, 1])
        x2 = np.minimum(box[2], src_boxes[:, 2])
        y2 = np.minimum(box[3], src_boxes[:, 3])
        pub_w = x2 - x1
        pub_h = y2 - y1
        match_idx_first = np.where((pub_w > 3) & (pub_h > 3))[0]
        diff_w_ratio = (boxes_w[match_idx_first] - box_w) / (box_w + 1e-6)
        # OCR框比layout宽度大于30%，则认为产生了分栏被意外合并
        match_idx = match_idx_first[np.where(diff_w_ratio <= 0.25)[0]]
        un_match_idx = match_idx_first[np.where(diff_w_ratio > 0.25)[0]]
        use_crop_ocr = True if len(un_match_idx) > 0 else False
        match_idx_list.extend(match_idx)
    return match_idx_list, use_crop_ocr


def get_sub_regions_ocr_res(overall_ocr_res, box, flag_within=True):
    """
    ...
    match_idx_list, use_crop_ocr = get_overlap_boxes_idx(overall_text_boxes, box)
    if use_crop_ocr:
        return None, use_crop_ocr
    ...
    return sub_regions_ocr_res, False
